...
class SWUpdate:
    ...
    def GET(self, *args, **kwargs):
        ...
        try:
            proc = run([SWUpdate.SWUPDATE_SCRIPT, "get-update", str(mode)], capture_output=True,
                       timeout=SystemSettingsManage.get_user_callback_timeout())
            if proc.returncode:
                result['InfoMsg'] = proc.stderr.decode("utf-8")
    ...
    def DELETE(self):
        if cherrypy.session.get('swupdate', None) == cherrypy.session.id:
            ...
            run([SWUpdate.SWUPDATE_SCRIPT, "post-update"])
            SWUpdate._isUpdating = False
