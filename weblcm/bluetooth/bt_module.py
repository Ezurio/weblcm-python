...
class BtMgr(threading.Thread):
        ...
        for path, interfaces in self.objects.items():
            if path.startswith(self.adapter.object_path):
                ...
                if device and str(device['Address']) == address:
                    ...
                    device = Device(address, path, self.characteristic_property_change_callback,
                                    self.mgr_connection_callback, self.write_notification_callback)
                    device.connect()
                    success = True
            ...
            if device is not None:
                ...
                if service:
                    ...
                    if char:
                        ...
                        self.logger.error('Characteristic UUID {} not found for service {} and device {}'.format(
    ...
    def configure_characteristic_notification(self, address, service_uuid, char_uuid, enable):
        ...
        """
        if enable:
            self.logger.info('Starting notifications for characteristic {} in service {} for device {}'.format(
                char_uuid, service_uuid, address))
        else:
            self.logger.info('Stopping notifications for characteristic {} in service {} for device {}'.format(
                char_uuid, service_uuid, address))
        ...
        try:
            ...
            self.logger.error(
                'Failed to configure characteristic notifications for device {} characteristic {}: {}'.format(address, char_uuid, e))
    ...
    def mgr_connection_callback(self, device):
        ...
        if data['connected']:
            ...
            self.logger.info('Added device {}, path {}, list count {}'.format(
...
class Service():
    ...
    def add_characteristic(self, uuid, path):
        char = Characteristic(uuid, path, self.write_characteristic_notification_callback,
...
class Characteristic():
    ...
    def __init__(self, uuid, path, property_change_callback, write_notification_callback=None):
        ...
        self.properties_signal = self.properties.connect_to_signal(
            'PropertiesChanged', self.characteristic_property_change_callback)
