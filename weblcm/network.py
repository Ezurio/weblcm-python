...
class NetworkConnections(object):
    ...
    def GET(self, *args, **kwargs):
        ...
        for conn in NetworkManager.NetworkManager.ActiveConnections:
            ...
            rconn = result.get('connections')
            if rconn and rconn.get(uuid):
                rconn[uuid]['activated'] = 1
                    ...
                    if post_data.get('802-11-wireless-security'):
                        ...
                        if post_data.get('802-1x'):
                            ...
                            if new_settings['802-1x'].get('ca-cert'):
                                new_settings['802-1x']['ca-cert'] = definition.FILEDIR_DICT.get(
                                    'cert') + new_settings['802-1x'].get('ca-cert')
                            if new_settings['802-1x'].get('client-cert'):
                                new_settings['802-1x']['client-cert'] = definition.FILEDIR_DICT.get(
                                    'cert') + new_settings['802-1x'].get('client-cert')
                            if new_settings['802-1x'].get('private-key'):
                                new_settings['802-1x']['private-key'] = definition.FILEDIR_DICT.get(
                                    'cert') + new_settings['802-1x'].get('private-key')
                            if new_settings['802-1x'].get('phase2-ca-cert'):
                                new_settings['802-1x']['phase2-ca-cert'] = definition.FILEDIR_DICT.get(
                                    'cert') + new_settings['802-1x'].get('phase2-ca-cert')
                            if new_settings['802-1x'].get('phase2-client-cert'):
                                new_settings['802-1x']['phase2-client-cert'] = definition.FILEDIR_DICT.get(
                                    'cert') + new_settings['802-1x'].get('phase2-client-cert')
                            if new_settings['802-1x'].get('phase2-private-key'):
                                new_settings['802-1x']['phase2-private-key'] = definition.FILEDIR_DICT.get(
                                    'cert') + new_settings['802-1x'].get('phase2-private-key')
    ...
    def GET(self, *args, **kwargs):
        ...
        try:
            if not Version._version:
                ...
                Version._version['weblcm_python_webapp'] = definition.WEBLCM_PYTHON_VERSION
                Version._version['build'] = subprocess.check_output(
                    "sed -n 's/^VERSION=//p' /etc/os-release", shell=True).decode('ascii').strip().strip('"')
