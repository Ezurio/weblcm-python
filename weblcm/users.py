...
from .settings import WeblcmConfigManage, SystemSettingsManage
...
class UserManageHelper(object):
    ...
    def verify(cls, username, password):
        key = WeblcmConfigManage.get_key_from_section(username, 'salt')
        if not key:
            return False
    ...
    def addUser(cls, username, password, permission=None):
        if WeblcmConfigManage.add_section(username):
            salt = uuid.uuid4().hex
            WeblcmConfigManage.update_key_from_section(username, 'salt', salt)
            WeblcmConfigManage.update_key_from_section(
                username, 'password', hashlib.sha256(salt.encode() + password.encode()).hexdigest())
            if permission:
                WeblcmConfigManage.update_key_from_section(username, 'permission', permission)
            return WeblcmConfigManage.save()
        return False
    ...
    def updatePermission(cls, username, permission):
        if permission and WeblcmConfigManage.get_key_from_section(username, 'permission', None):
            return WeblcmConfigManage.update_key_from_section(username, 'permission', permission)
        return False
...
class LoginManageHelper(object):
    ...
    def login_failed(cls, username):
        with cls._lock:
            ...
            user = cls._failed_logins.get(username, {})
            if user:
                user['time'] = [dt for dt in user['time'] if abs(
                    (now - dt).total_seconds()) < SystemSettingsManage.get_login_retry_window()]
