...
print('Build: ' + subprocess.check_output(['cat','/etc/laird-release']).decode('ascii').rstrip())
...
for d in devices:
	...
	print('\tFirmware Version: ' + str(prop_iface.Get(NM_DEVICE_IFACE, "FirmwareVersion")))
	...
	if device_type == 2:
		...
		print('\tSupplicant: ' + subprocess.check_output(['sdcsupp','-v']).decode('ascii').rstrip())
		#NM driver version only returns kernel version, which matches modinfo vermagic
		print('\tDriver Version: ' + subprocess.check_output(['modinfo','--field=version',driver]).decode('ascii').rstrip())